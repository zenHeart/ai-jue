...
describe("hermes skill-plugin", () => {
  ...
  it("refuses marker-like comments with unrelated user code", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "jue-hermes-skill-plugin-marker-"));
    roots.push(root);
    fs.writeFileSync(
      path.join(root, "__init__.py"),
      '# Auto-generated by ai-jue\nUSER_CODE = "preserve"\n',
    );
    await expect(
      write({ skills: {} }, { scope: "project", artifactRoot: root, artifactKind: "skill-plugin" }),
    ).rejects.toThrow("hand-authored __init__.py");
...
describe("detectArtifactKind", () => {
  ...
  it("does not claim a layout from marker-like initializer text", () => {
