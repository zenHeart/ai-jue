...
| Cursor | Done | Done | Done (`packages/ai-jue-adapter-cursor/`) — project and Plugin Artifacts; Skills/Subagents/Commands keep frontmatter; project hooks use `{ version: 1, hooks }`, plugin hooks use `{ hooks }`; command MCP servers get `type: "stdio"`; `variables` passthrough via `tools.cursor.pluginManifest`; `tools.cursor.marketplace` writes and validates a multi-plugin repository index, Adapter Read validates the index-to-child relation before Canonical projection, and the exported `readMarketplaceIndex()` provides the target-private index. `confirm()` returns `unconfirmed` for both kinds (plugin includes structural evidence) |
...
`tools.cursor.marketplace` now provides a multi-plugin repository index with
index-to-child manifest validation. These GitHub Issues are **separate follow-ups** — agents must read the
full issue (Acceptance criteria + Implementation notes) before starting:
