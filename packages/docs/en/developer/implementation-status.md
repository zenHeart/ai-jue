...
| --- | --- | --- | --- |
| Codex | Done | Done | Done (`packages/ai-jue-adapter-codex/`, JUE-301) — capability declaration honest about the three unsupported boundaries: `commands: "degraded"` (Codex's custom-commands mechanism deprecated per JUE-104/105/JUE-301 Phase 1), `mcp: "degraded"` (Codex MCP config lives in `[mcp_servers.*]` TOML tables, out of scope for the JSON-based factory), `rules: "degraded"` (no separate rules directory; rules fold into AGENTS.md via the `context` mapping). Native confirmation uses the real `codex plugin marketplace add <local> --marketplace <name>` + `codex plugin add <name> --marketplace <name>` + `codex plugin list --json` round-trip (isolated CODEX_HOME); Codex 0.145.0 has no `codex plugin validate` so this is the strongest native confirmation Codex currently offers. `scripts/verify-codex-native.js` replays the full read→write→applyChangesOrThrow→confirm chain against the real codex CLI 0.145.0. |
