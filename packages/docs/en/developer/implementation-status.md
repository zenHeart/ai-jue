...
OpenClaw compatible-bundle and Hermes skill-plugin confirmation now validate
managed regular-file/realpath boundaries before execution, use minimal
environments and isolated state, and require the native inventory to contain
the installed identity. Hermes confirms through a temporary local Git source
with `plugins install` plus `list --user --json`; OpenClaw uses non-interactive
install, list, and inspect and requires the inspected format to match the
generated marker. Both paths were confirmed with current CLIs on authorized
