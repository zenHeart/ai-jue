...
| [RFC-0004：项目层 cross-tool symlink 模式的可移植发现合同](0004-portable-discovery-cross-tool-symlink.md) | Proposed | 在 `jue inspect --diagnostics` 暴露 link-pattern 检查；同步把「物化优于 symlink」写进 `documentation-contract.md` / `creating-a-preset.md`；`jue materialize-cross-links` 暂缓 |
