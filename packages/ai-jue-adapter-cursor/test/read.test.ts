...
describe("cursor read()", () => {
  it("reads the project fixture into Canonical DSL", async () => {
    const canonical = await read({ scope: "project", artifactRoot: path.join(FIXTURES_ROOT, "project") });
  ...
  it("reads the plugin fixture into Canonical DSL without context.global", async () => {
    const canonical = await read({ scope: "project", artifactRoot: path.join(FIXTURES_ROOT, "plugin") });
  ...
  it("reads the minimal plugin fixture", async () => {
    const canonical = await read({ scope: "project", artifactRoot: path.join(FIXTURES_ROOT, "plugin-minimal") });
  ...
  it("falls back to project layout when no layout marker exists", async () => {
    ...
    const canonical = await read({ scope: "project", artifactRoot: root });
