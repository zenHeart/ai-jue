...
export const builder: CommandBuilder = (yargs) => {
  return yargs.positional("type", {
    describe: t("commands.list.type_describe", {
      defaultValue: "Type of assets to list",
    }),
    type: "string",
...
export const handler = async (argv: Arguments) => {
  ...
  if (type === "presets" || type === "all") {
    ...
    logger.info(t("commands.list.active_presets"));
    if (presets.length > 0) {
      presets.forEach((p: string) => logger.log(`- ${p}`));
    } else {
      logger.log(`  ${t("commands.list.none", { defaultValue: "(none)" })}`);
    }
  }

  if (type === "prompts" || type === "all") {
    logger.info(
      t("commands.list.prompts_header", { defaultValue: "\nLoaded Prompts:" }),
    );
  ...
  if (type === "skills" || type === "all") {
    logger.info(
      t("commands.list.skills_header", { defaultValue: "\nLoaded Skills:" }),
    );
