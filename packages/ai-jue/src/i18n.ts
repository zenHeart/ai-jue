  ...
  en: {
    translation: {
      ...
      commands: {
        ...
        format: {
          describe: "Migrate existing AI tool configs to .ai directory",
          write_describe: "Execute migration (default: dry-run)",
          force_describe: "Force overwrite existing files",
          ...
          summary_header: "\nSummary:",
          summary_total: "  Total items: {{total}}",
          summary_safe: "  Safe to migrate: {{count}}",
          summary_conflicts: "  Conflicts: {{count}}",
          summary_review: "  Needs manual review: {{count}}",
          items_header: "\nItems to migrate:\n",
          conflict_label: "Conflict: {{resolution}}",
          dry_run_hint: "\n📌 This is a dry-run. Use --write to execute migration.",
          conflict_warn: "\n⚠️  {{count}} conflict(s) detected. Use --force to overwrite, or resolve manually.",
          migrating: "Migrating configurations...",
          migrated_item: "Migrated: {{source}} → {{target}}",
          skipped_conflict: "Skipped (conflict): {{source}}",
          migration_success: "✨ Migration completed!",
          next_steps_header: "\nNext steps:",
          next_step_review: "  1. Review the migrated files in .ai/",
          next_step_config: "  2. Update your ai.config.js to use the new structure",
          next_step_apply: "  3. Run 'npx jue apply' to generate tool-specific configs",
          migration_failed: "Migration failed: {{message}}",
        },
      },
    },
  },
  zh: {
    translation: {
      ...
      commands: {
        ...
        format: {
          describe: "将现有 AI 工具配置迁移到 .ai 目录",
          write_describe: "执行迁移（默认为 dry-run）",
          force_describe: "强制覆盖现有文件",
          ...
          summary_header: "\n概览：",
          summary_total: "  总计项目：{{total}}",
          summary_safe: "  可安全迁移：{{count}}",
          summary_conflicts: "  冲突项：{{count}}",
          summary_review: "  需人工复核：{{count}}",
          items_header: "\n待迁移项目：\n",
          conflict_label: "冲突：{{resolution}}",
          dry_run_hint: "\n📌 当前为 dry-run 模式。使用 --write 执行实际迁移。",
          conflict_warn: "\n⚠️  检测到 {{count}} 处冲突。使用 --force 强制覆盖，或手动处理。",
          migrating: "正在迁移配置...",
          migrated_item: "已迁移：{{source}} → {{target}}",
          skipped_conflict: "已跳过（冲突）：{{source}}",
          migration_success: "✨ 迁移完成！",
          next_steps_header: "\n后续步骤：",
          next_step_review: "  1. 检查 .ai/ 目录下的迁移文件",
          next_step_config: "  2. 更新 ai.config.js 以启用新结构",
          next_step_apply: "  3. 运行 'npx jue apply' 生成工具特定的配置文件",
          migration_failed: "迁移失败：{{message}}",
        },
      },
    },
  },
